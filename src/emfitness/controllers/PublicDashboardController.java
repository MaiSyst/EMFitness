...
public class PublicDashboardController {
    ...
    public PublicDashboardController(
        ...
        saveMember.addActionListener(l -> addNewMember(Integer.parseInt(
                priceSubscription.getText().replace(".", "").split(" ")[0])));
        subscription.addItemListener(itemListener -> {
            if (itemListener.getStateChange() == ItemEvent.SELECTED) {
                ...
                if (!price.isBlank()) {
                    var substraction = Integer.parseInt(price) - Integer.parseInt(priceSubscription.getText()
                            .replace(".", "")
                            .split(" ")[0]);
                    if (substraction >= 0) {
                        ...
                        fetch.post(
                                ...
                                body).then((result, status) -> {
                                    ...
                                    if (status == ResponseStatusCode.OK) {
                                        ...
                                        AtomicReference<String> expireDate=new AtomicReference();
                                        customerModel.subscribes().forEach(expired->{
                                            if(!isExpired(expired.dateEnd())){
                                                expireDate.set(expired.dateEnd());
                                            }
                                        });
    ...
    private boolean isExpired(String dateEnd) {
        MaiDateResponse response = MaiDate.compareBetween(Date.valueOf(dateEnd));
        return response.getInfoDays() == MaiDateInfoType.PASSED || response.getInfoDays() == MaiDateInfoType.EXPIRED;
    }
